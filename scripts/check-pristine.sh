#!/bin/sh
# check-pristine.sh — fail if any PUBLIC (committable/packageable) file contains a
# site-private string. This keeps the repo and the .deb pristine (design §12).
#
# The denylist of private strings is itself private, so it is NOT hardcoded here
# (that would leak what we guard). It is discovered, in order:
...
# (one extended-regex per line; '#' comments ok). If none is available the check is
# SKIPPED with a warning (e.g. on a fresh clone) rather than giving false comfort.
set -eu
...
if [ -z "$patterns" ] || [ ! -r "$patterns" ]; then
  echo "check-pristine: WARNING: no denylist found — skipping (cannot verify)" >&2
  exit 0
fi
...
if [ -z "$clean" ]; then
  echo "check-pristine: WARNING: denylist has no usable patterns — skipping" >&2
  exit 0
fi
...
echo "check-pristine: OK — no site-private strings in public files"
